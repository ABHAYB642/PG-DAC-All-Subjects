...
public class ProductDaoImpl implements ProductDao{
    ...
    static PreparedStatement selprod,delprod,insprod,selById,upById,findByPrice,selByPrice;
    static{
    	try {
			...
			findByPrice=conn.prepareStatement("select distinct price from product");
	...
	public List<Double> findAllPrices() {
		List<Double> prlst=new ArrayList<>();
		try {
			ResultSet rs=findByPrice.executeQuery();
			while(rs.next()) {
				prlst.add(rs.getDouble(1));
			}
			return prlst;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	@Override
	public List<Product> findAllProductsByPrice(double pr) {
		List<Product> plist=new ArrayList<>();
	try {
		selByPrice.setDouble(1, pr);
